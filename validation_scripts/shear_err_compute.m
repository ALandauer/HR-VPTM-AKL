clear
close all
load('C:\Users\akl1\Desktop\LFM\local\data\Shear_synthetic_volume\xz\results_complete_xz_20210415.mat')
init_config = load('C:\Users\akl1\Desktop\LFM\local\data\Shear_synthetic_volume\simple_shear_25perc_syn.mat');
...
for ImgSeqNum = 2:length(file_names)
    ...
    parCoordA_GT{ImgSeqNum-1}(:,3) = parCoordACum{ImgSeqNum-1}(:,3) + 26;% MPTPara.axesScale(3)*sizeV(3)/2;
...
figure
plot3(parCoordA_GT{1}(:,2),parCoordA_GT{1}(:,1),parCoordA_GT{1}(:,3),'or')
hold on
...
%     parCoordBCum_vec = parCoordBCum_GT{ii};
    ...
    eij_GT{ii}{3,3} = reshape(F_Grid_refB_Vector_GT(9:9:end),size(x_Grid_refB));
    eij_GT{ii}{1,2} = 2*reshape(0.5*(F_Grid_refB_Vector_GT(2:9:end)+F_Grid_refB_Vector_GT(4:9:end)),size(x_Grid_refB));
    eij_GT{ii}{1,3} = 2*reshape(0.5*(F_Grid_refB_Vector_GT(3:9:end)+F_Grid_refB_Vector_GT(7:9:end)),size(x_Grid_refB));
